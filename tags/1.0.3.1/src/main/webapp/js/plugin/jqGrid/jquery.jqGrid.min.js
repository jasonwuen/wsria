...
d+'","rules":[':{groupOp:d,rules:[]};a.find(".sf").each(function(c){var g=encodeURIComponent(jQuery(this).find("select[name='field'] :selected").val()),i=encodeURIComponent(jQuery(this).find("select[name='op'] :selected").val()),h=encodeURIComponent(jQuery(this).find("input.vdata,select.vdata :selected").val());h+="";h=h.replace(/\\/g,"\\\\").replace(/\"/g,'\\"');if(f.stringResult){if(c>0)b+=",";b+='{"field":"'+g+'",';b+='"op":"'+i+'",';b+='"data":"'+h+'"}'}else b.rules.push({field:g,op:i,data:h})});if(f.stringResult)b+="]}";f.onSearch(b);return false});e.find(".ui-reset").click(function(a,
